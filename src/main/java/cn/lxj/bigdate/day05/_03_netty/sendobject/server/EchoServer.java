...
import cn.lxj.bigdate.day05._03_netty.sendobject.coder.PersonDecoder;
            ...
            serverBootstrap.group(eventLoopGroup).channel(NioServerSocketChannel.class).localAddress("localhost", port)
                    .childHandler(new ChannelInitializer<Channel>() {
                        ...
                        protected void initChannel(Channel channel) throws Exception {
                            // 注册解码的handler
                            channel.pipeline().addLast(new PersonDecoder());// IN1 反序列化
                            // 添加一个入站的handler到ChannelPipeline
                            channel.pipeline().addLast(new EchoServerHandler());//IN2
                        }
                    });
