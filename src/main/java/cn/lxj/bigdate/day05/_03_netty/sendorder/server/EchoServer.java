        ...
        serverBootstrap.group(eventLoopGroup)
                ...
                .childHandler(new ChannelInitializer<Channel>() {//设置childHandler执行所有的连接请求
                    ...
                    protected void initChannel(Channel ch) throws Exception {
                        ...
                        ch.pipeline().addLast(new EchoInHandler1());    // in正序执行
                        ch.pipeline().addLast(new EchoOutHandler1());   // 有flush的out要在out的最前面
                        ch.pipeline().addLast(new EchoOutHandler2());   // out倒序执行
                        ch.pipeline().addLast(new EchoInHandler2());    // 有write的要放在in的最后
                    }
                });
